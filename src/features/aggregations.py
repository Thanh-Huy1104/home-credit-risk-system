"""Feature aggregation functions using DuckDB."""
...
def aggregate_bureau(con: duckdb.DuckDBPyConnection, data_raw: str) -> dict[str, Any]:
    ...
    file_path = f"{data_raw}/bureau.csv"
    ...
    return {"table": "bureau_agg", "rows": n_rows}


def aggregate_bureau_balance(con: duckdb.DuckDBPyConnection, data_raw: str) -> dict[str, Any]:
    """Aggregate bureau balance data by SK_ID_CURR.
    ...
    bureau_path = f"{data_raw}/bureau.csv"
    bb_path = f"{data_raw}/bureau_balance.csv"
    ...
    logger.info(f"   {n_rows:,} rows")
    return {"table": "bureau_balance_agg", "rows": n_rows}


def aggregate_previous_application(con: duckdb.DuckDBPyConnection, data_raw: str) -> dict[str, Any]:
    ...
    file_path = f"{data_raw}/previous_application.csv"
    ...
    logger.info(f"   {n_rows:,} rows")
    return {"table": "prev_app_agg", "rows": n_rows}


def aggregate_installments_payments(
    con: duckdb.DuckDBPyConnection, data_raw: str
) -> dict[str, Any]:
    ...
    file_path = f"{data_raw}/installments_payments.csv"
    ...
    logger.info(f"   {n_rows:,} rows")
    return {"table": "installments_agg", "rows": n_rows}


def aggregate_pos_cash(con: duckdb.DuckDBPyConnection, data_raw: str) -> dict[str, Any]:
    """Aggregate POS cash balance data by SK_ID_CURR.
    ...
    file_path = f"{data_raw}/POS_CASH_balance.csv"
    ...
    logger.info(f"   {n_rows:,} rows")
    return {"table": "pos_cash_agg", "rows": n_rows}


def aggregate_credit_card(con: duckdb.DuckDBPyConnection, data_raw: str) -> dict[str, Any]:
    """Aggregate credit card balance data by SK_ID_CURR.
    ...
    file_path = f"{data_raw}/credit_card_balance.csv"
    ...
    logger.info(f"   {n_rows:,} rows")
    return {"table": "credit_card_agg", "rows": n_rows}


def join_all_features(con: duckdb.DuckDBPyConnection, processed_dir: str) -> dict[str, Any]:
    """Join all aggregated features to master application table.
    ...
    """
    logger.info("Joining all tables to master...")
    ...
    logger.info(f"   {n_rows:,} rows x {n_cols:,} columns in application_features")

    return {"rows": n_rows, "columns": n_cols}
